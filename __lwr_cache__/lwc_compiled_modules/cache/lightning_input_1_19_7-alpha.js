...
import { FieldConstraintApiWithProxyInput, InteractingState, normalizeVariant, VARIANT, normalizeInput, isValidEmail, isValidMultipleEmails } from 'lightning/inputUtils';
const i18n = {
...
class LightningInput extends LightningElement {
  ...
  set value(value) {
    ...
    if (!this.isTypeFile) {
      ...
      } else if (this._rendered && this._inputElement.value !== this._displayedValue) {
  ...
  get _primitiveComponent() {
    if (!this.cachedPrimitiveComponent || this._primitiveComponentRefreshNeeded) {
  ...
  _validateType(type) {
    ...
    if (type === 'phone') {
      console.warn(`<lightning-input> The type attribute value "phone" is invalid. Please use <lightning-input type="tel"> instead. Defaulting to text type.`);
    }
